...
func (slicer *Slicer) ShakeHolesFill() {
	...
	for inx, hole := range holes {
		...
		fmt.Printf("Shake Nil-Cells=%d %d/%d\n", hole.CountNilCells(), holesLen, inx)

		holeQueue := InitCoordinateQueue()
		holeQueue.PushAll(hole.Traversal())

		slicer.tryDestuctShinkWithQueue(holeQueue)

		if hole.CountNilCells() <= 0 {
			continue
		}
		...
		lefts := slicer.ShakeSlicesWithQueueFeedBack(queue)
		coords = coords.Union(lefts)
	}

	//fmt.Println()

	destQueue := InitCoordinateQueue()
	...
	slicer.tryDestuctShinkWithQueue(destQueue)
}